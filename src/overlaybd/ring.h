...
public:
    ...
    uint32_t size() const {
        return mod(m_end - m_begin);
    }
    bool empty() const {
        return m_begin == m_end;
    }
    bool full() const {
        return next_end() == m_begin;
    }
    void wait_for_push(uint64_t timeout = -1) {
        m_cond_push.wait_no_lock(timeout);
    }
    void wait_for_pop(uint64_t timeout = -1) {
        m_cond_pop.wait_no_lock(timeout);
    }
...
protected:
    ...
    uint32_t mod(uint32_t i) const {
        return i & m_mask;
    }
    uint32_t next_end() const {
        return mod(m_end + 1);
    }
    uint32_t next_begin() const {
        return mod(m_begin + 1);
    }
};
...
public:
    ...
    T &back() {
        return m_buf[mod(m_end - 1)];
    }
    T &operator[](uint64_t i) {
        return m_buf[mod(m_begin + i)];
    }
    const T &front() const {
        return m_buf[m_begin];
    }
