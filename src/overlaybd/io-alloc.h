...
struct IOAlloc {
    ...
    IOAlloc() : allocate(nullptr, &default_allocator), deallocate(nullptr, &default_deallocator) {
    }

    IOAlloc(Allocator alloc, Deallocator dealloc) : allocate(alloc), deallocate(dealloc) {
    }
...
struct AlignedAlloc : public IOAlloc {
    AlignedAlloc(size_t alignment)
        : IOAlloc(Allocator{(void *)alignment, &aligned_allocator},
...
template <size_t MAX_ALLOCATION_SIZE = 1024 * 1024, size_t SLOT_CAPACITY = 32,
          size_t ALIGNMENT = 4096>
class PooledAllocator {
    constexpr static bool is_power2(size_t n) {
        return (n & (n - 1)) == 0;
    }
...
protected:
    ...
    public:
        Slot()
            : IdentityPool<void, SLOT_CAPACITY>({this, &Slot::myalloc},
                                                {this, &IOAlloc::default_deallocator}) {
        }
