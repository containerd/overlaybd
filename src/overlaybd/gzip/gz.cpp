...
// class StreamFile : public photon::fs::IFile {
// public:

// }

class GzStreamFile : public GzAdaptorFile {
public:
    GzStreamFile(IStream *sock, ssize_t st_size, IFile *index_save)
        : sock(sock), st_size(st_size), m_idx_file(index_save) {
        ...
        buffer_fs = photon::fs::new_localfs_adaptor();
        buffer_file = buffer_fs->open(FN_BUFF_FILE, O_TRUNC | O_CREAT | O_RDWR);

        LOG_INFO("create a GzStreamFile. expected size: `", st_size);
    };

    ~GzStreamFile() {
        (void)inflateEnd(&strm);
        buffer_fs->unlink(FN_BUFF_FILE);
    }
    ...
    ssize_t read(void *buf, size_t count) override {
        ...
        while (count > 0) {
            ...
            strm.avail_in = sock->read(in, CHUNK);
            if (strm.avail_in < 0) {
    ...
    int save_index() {
        return save_index_to_file(m_idx_header, m_indexes, m_idx_file);
    }
    ssize_t st_size = -1;
    ...
    size_t have = 0;
    off_t ttin, ttout, bf_start, bf_len;
    off_t cur_offset = 0;
    photon::fs::IFileSystem *buffer_fs = nullptr;
    IFile *buffer_file = nullptr;
    IFile *m_idx_file = nullptr;
    const char *FN_BUFF_FILE = "/tmp/decompbuffer";
...
photon::fs::IFile *open_gzstream_file(IStream *sock, ssize_t st_size,
                                      photon::fs::IFile *save_idx_as) {
    return new GzStreamFile(sock, st_size, save_idx_as);
}

int save_gzip_index(photon::fs::IFile *file) {
    return ((GzStreamFile *)file)->save_index();
}