...
int libcurl_init(long flags = 3 /*CURL_GLOBAL_DEFAULT*/, long pipelining = 0, long maxconn = 32);
...
class BufWriter {
public:
    BufWriter(void *buf_, size_t capacity_) : m_capacity(capacity_), m_buf(buf_) {
    }
    size_t write(const void *ptr, size_t n) {
    ...
    ALogString alog_string() {
        return ALogString((const char *)m_buf, m_size);
    }
...
public:
    ...
    ALogStringSTD alog_string() {
        return ALogStringSTD(string);
    }
};
...
struct IOVWriter : public IOVector {
    ...
    size_t drop = 0;
    size_t write(const void *buf, size_t size) {
        auto actual_count = memcpy_from(buf, size); // copy from (buf, size) to iovec[] in *this,
...
public:
    BufReader(const void *buf_, size_t capacity_)
        : m_size(capacity_), m_capacity(capacity_), m_buf((char *)buf_ + capacity_) {
    }
...
public:
    ...
    void snp_append(const char *fmt, const Ts &...xs) {
        if (!overflow())
            m_size += snprintf((char *)m_url + m_size, m_capacity - m_size, fmt, xs...);
    }
...
public:
    ...
    static int init(long flags = CURL_GLOBAL_ALL, long pipelining = 0, long maxconn = 32) {
    ...
    cURL &setopt(CURLoption option, long arg) {
        return _setopt(option, arg);
    }
    cURL &setopt(CURLoption option, void *arg) {
        return _setopt(option, arg);
    }
    ...
    cURL &set_nobody() {
        return setopt(CURLOPT_NOBODY, 1);
    }
    ...
    cURL &set_cafile(const char *cafile) {
        long v = (bool)cafile;
        return setopt(CURLOPT_CAINFO, cafile), setopt(CURLOPT_SSL_VERIFYPEER, v);
    }
    cURL &set_redirect(uint32_t max_redir) {
        long v = (bool)max_redir;
        return setopt(CURLOPT_FOLLOWLOCATION, v), setopt(CURLOPT_MAXREDIRS, max_redir);
    }
    template <typename T>
    ...
    cURL &set_verbose(bool on = true) {
        if (on) {
            ...
            return setopt(CURLOPT_STDERR, (void *)file), setopt(CURLOPT_VERBOSE, 1L);
        } else {
    ...
    template <typename W>
    long POST(const char *url, const char *post_fields, W *wstream, uint64_t timeout = -1) {
        setopt(CURLOPT_HTTPHEADER, headers.list);
...
protected:
    struct slist {
        ...
        void append(const char *h) {
            list = curl_slist_append(list, h);
        }
        void append(const std::string &h) {
            append(h.c_str());
        }
        void clear() {
    ...
    cURL &_setopt(CURLoption option, T arg) {
        ...
        if (ret != CURLE_OK)
            LOG_WARN("failed to curl_easy_setopt(m_curl, option=`, arg=`)", option, arg);
        return *this;
    }
    template <typename T>
