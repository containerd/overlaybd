...
namespace FileSystem
{
    ...
    public:
        ...
        static void perform(uint64_t /*timeout*/, const F0& act)
        {
            ...
            safe_thread_interrupt(evloop->loop_thread(), EINPROGRESS);
        }
        static int wait4events(void*, EventLoop*)
        {
            if (op_queue.read_available() == 0)
            {
                int ret = photon::thread_usleep(10 * 1000);
                if (ret < 0)
                {
        ...
        void do_callback(uint32_t id, RT result, Done<R> done)
        {
            ...
            if (op_failed(ar.result))
            {
                ...
                LOG_ERROR("failed to perform an action (ID = `) ", id, e);
            }
    ...
    public:
        ...
        OVERRIDE_ASYNC(int, fallocate, int mode, off_t offset, off_t length)
        {
            PERFORM(OPID_FALLOCATE, m_file->fallocate(mode, offset, length));
        }
    };
