...
#define _this static_cast<const Derived *>(this)
    void divide(uint64_t x, uint64_t &round_down, uint64_t &remainder, uint64_t &round_up) const {
...
#undef _this
    ...
    __attribute__((always_inline)) void init(uint64_t offset, uint64_t length) {
        begin = offset;
        end = offset + length;
        ...
        if (abegin + 1 == aend) {
            ...
            if (abegin == apbegin)
                ...
                preface.assign(abegin, begin_remainder, get_length(abegin) - begin_remainder);
            if (preface)
                first = preface;
            else
            ...
            if (aend == apend)
                ...
                postface.assign(apend, 0, end_remainder);
        }
    }
    ...
    struct aligned_parts_t {
        ...
        struct iterator : sub_range {
            ...
            iterator(const basic_range_split *split, uint64_t i)
                : sub_range(i, 0, split->get_length(i)), split(split) {
            } // prevent initialize order warning
        ...
        iterator end() const {
            if (split->small_note) // small note have no aligned parts, but apbegin > apend (means
                                   // empty)
                return iterator(
                    split,
                    split->apbegin); // therefore, end() should return apbegin for range-based loop
...
struct range_split_power2 : public basic_range_split<range_split_power2> {
    ...
    range_split_power2(uint64_t offset, uint64_t length, uint64_t interval) : interval(interval) {
