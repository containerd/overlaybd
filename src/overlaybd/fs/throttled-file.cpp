...
public:
    ...
    void push_back(uint32_t amount) {
        ...
        if (rate()) {
            while (sum() >= limit()) {
                uint64_t next_check =
                    _get_time(m_events.front().time_stamp) + m_time_window * 1024UL;
                if (next_check > now)
...
protected:
    inline __attribute__((always_inline)) void _update_timestamp_base(uint64_t now) {
    ...
    inline __attribute__((always_inline)) uint64_t _get_time(uint32_t timestamp) {
        return m_timestamp_base + timestamp;
    }

    inline __attribute__((always_inline)) uint32_t _get_stamp(uint64_t timems) {
...
struct scoped_queue {
    ...
    uint32_t _amount;
    uint64_t _ts_end;
    scoped_queue(StatisticsQueue &q, uint64_t count) : _q(q), _amount((uint32_t)(count)) {
        q.push_back(_amount);
...
struct scoped_semaphore {
    ...
    scoped_semaphore(photon::semaphore &sem, uint64_t count) : m_count(count), m_sem(sem) {
        m_sem.wait(m_count);
    }
    ~scoped_semaphore() {
...
split_io(ALogStringL name, size_t count, size_t block_size, const Func &func, const Adv &adv) {
...
public:
    struct Throttle {
        ...
        Throttle(const ThrottleLimits::UpperLimits &limits, uint32_t window)
            : num_io(limits.concurent_ops
                         ? limits.concurent_ops
                         : UINT32_MAX), // 0 for no limit, uint32_max to avoid overflow
              iops(limits.IOPS, window * 1024U), throughput(limits.throughput, window * 1024U) {
        }
    };
    ...
    struct scoped_throttle {
        ...
        scoped_throttle(Throttle &t1, Throttle &t2, uint64_t cnt)
            : count(cnt), sem1(t1.num_io, 1), sem2(t2.num_io, 1), q11(t1.iops, 1), q12(t2.iops, 1),
              q21(t1.throughput, count), q22(t2.throughput, count) {
        }
    ...
    ThrottledFile(IFile *file, const ThrottleLimits &limits)
        : ForwardFile(file), m_limits(limits), t_all(limits.RW, limits.time_window),
          t_read(limits.R, limits.time_window), t_write(limits.W, limits.time_window) {
    }

    virtual ssize_t pread(void *buf, size_t count, off_t offset) override {
        scoped_throttle t(t_all, t_read, count);
        return split_io(
            __func__, count, m_limits.R.block_size,
    ...
    virtual ssize_t preadv(const struct iovec *iov, int iovcnt, off_t offset) override {
        ...
        return split_io(
            __func__, v.count, m_limits.R.block_size,
    ...
    virtual ssize_t pwrite(const void *buf, size_t count, off_t offset) override {
        ...
        return split_io(
            __func__, count, m_limits.W.block_size,
    ...
    virtual ssize_t pwritev(const struct iovec *iov, int iovcnt, off_t offset) override {
        ...
        scoped_throttle t(t_all, t_read, v.count);
        return split_io(
            __func__, v.count, m_limits.W.block_size,
