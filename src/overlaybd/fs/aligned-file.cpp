...
public:
    ...
    AlignedFileAdaptor(IFile *underlay_file, uint32_t alignment, bool align_memory, bool ownership,
                       IOAlloc *allocator)
    ...
    virtual ssize_t pread(void *buf, size_t count, off_t offset) override {
        ...
        if (ret < (ssize_t)(rs.begin_remainder)) {
            LOG_ERRNO_RETURN(
                0, -1, "failed to aligned [`]->pread(buf=`, count=`, offset=`), ret: ` ( < ` )",
                m_file, ptr, rs.aligned_length(), rs.aligned_begin_offset(), ret,
                rs.begin_remainder + count);
        }
    ...
    virtual ssize_t pwrite(const void *buf, size_t count, off_t offset) override {
        ...
        if (rs.begin_remainder > 0) {
            ...
            if ((ret < 0) || ((off + ret < filesize) && (ret < (ssize_t)m_alignment)))
                LOG_ERRNO_RETURN(
                    0, -1,
                    "failed to aligned [`]->pread(ptr=`, count=`, offset=`) and patch the first alignment block",
                    m_file, ptr, m_alignment, off);
        ...
        if (!rs.small_note && rs.end_remainder > 0) {
            ...
            if (filesize - off > (ssize_t)(rs.end_remainder)) {
                ...
                if ((ret < 0) || ((ret + off < filesize) &&
                                  (ret < (ssize_t)m_alignment))) // cannot fetch all data of file,
                                                                 // and cannot fillup aligned block
                    LOG_ERRNO_RETURN(
                        0, -1,
                        "failed to aligned [`]->pread(ptr=`, count=`, offset=`) and patch the last alignment block",
                        m_file, ptr_, m_alignment, off);
            }
        }
    ...
    virtual ssize_t preadv_mutable(struct iovec *iov, int iovcnt, off_t offset) override {
        ...
        ret = m_file->preadv(rbuf.iovec(), rbuf.iovcnt(), rs.aligned_begin_offset());
        if (ret < rs.begin_remainder) {
            LOG_ERRNO_RETURN(
                0, -1, "failed to aligned [`]->pread(iov=`, iovcnt=`, offset=`), ret: ` ( < ` )",
                m_file, rbuf.iovec(), rbuf.iovcnt(), rs.aligned_begin_offset(), ret,
                rs.begin_remainder + count);
        }
        auto actual_read = ret - rs.begin_remainder;
        if (actual_read > count)
    ...
    virtual ssize_t pwritev_mutable(struct iovec *iov, int iovcnt, off_t offset) override {
        ...
        if (rs.begin_remainder > 0) {
            ...
            ssize_t off = rs.aligned_begin_offset();
            ret = m_file->preadv(view.iov, view.iovcnt, off);
            if ((ret < 0) || ((off + ret < filesize) && (ret < (ssize_t)m_alignment)))
                LOG_ERRNO_RETURN(
                    0, -1,
                    "failed to aligned [`]->preadv(iov=`, iovcnt=`, offset=`) and patch the first alignment block",
                    m_file, view.iov, view.iovcnt, off);
            if (ret < (ssize_t)m_alignment) {
            ...
            if (filesize - off > (ssize_t)(rs.end_remainder)) {
                ...
                if (ret < 0)
                    LOG_ERRNO_RETURN(0, -1,
                                     "failed to aligned [`]->preadv(ptr=`, count=`, offset=`)",
                                     m_file, view.iov, view.iovcnt, off);
                if ((ret + off < filesize) &&
                    (ret < (ssize_t)m_alignment)) // cannot fetch all data of file, and cannot
                                                  // fillup aligned block
                    LOG_ERRNO_RETURN(0, -1,
                                     "aligned [`]->preadv(ptr=`, count=`, offset=`) partial read",
                                     m_file, view.iov, view.iovcnt, off);
            }
        }
        view.iovcnt = MAX_TMP_IOVEC_SIZE;
        ...
        assert(ret == count);
        auto actual_write =
            m_file->pwritev_mutable(wbuf.iovec(), wbuf.iovcnt(), rs.aligned_begin_offset());
        auto current_tail = rs.aligned_begin_offset() + actual_write;
        if ((ssize_t)current_tail < offset)
            LOG_ERRNO_RETURN(0, -1, "failed to pwritev");
        if (suppose_filesize < current_tail) { // that means have written more than needs
            m_file->ftruncate(suppose_filesize);
...
public:
    ...
    bool m_align_memory;
    IOAlloc *m_allocator;
    AlignedFSAdaptor(IFileSystem *underlay_fs, uint32_t alignment, bool align_memory,
                     bool ownership, IOAlloc *allocator)
        : ForwardFS_Ownership(underlay_fs, ownership), m_alignment(alignment),
          m_align_memory(align_memory), m_allocator(allocator) {
    }
    virtual IFile *open(const char *path, int flags, mode_t mode) override {
        auto file = m_fs->open(path, flags, mode);
        if (file == nullptr)
            return nullptr;
        return new AlignedFileAdaptor(file, m_alignment, m_align_memory, true, m_allocator);
    }
...
IFile *new_aligned_file_adaptor(IFile *file, uint32_t alignment, bool align_memory, bool ownership,
                                IOAlloc *alloc) {
    if (file == nullptr)
...
IFileSystem *new_aligned_fs_adaptor(IFileSystem *fs, uint32_t alignment, bool align_memory,
                                    bool ownership, IOAlloc *alloc) {
    if (!is_power_of_2(alignment))
        LOG_ERROR_RETURN(EINVAL, nullptr, "alignment must be 2^n");
