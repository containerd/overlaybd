...
public:
    ...
    int init() override {
        ...
        for (auto file_path : enumerable(FileSystem::Walker(m_fs, ""))) {
            NsInfo info;
...
private:
    ...
    int get_ns_info(estring_view file_path, NsInfo &info) {
        ...
        if (format.checksum != FileSystem::crc32::crc32c(&format.info, sizeof(format.info))) {
    ...
    int append_ns(estring_view file_path, size_t file_size, NsInfo &info) {
        ...
        if (0 < pos && pos < file_path.length()) {
            ...
            if (m_fs->access(base_dir.data(), F_OK) != 0 &&
                FileSystem::mkdir_recursive(base_dir, m_fs) != 0) {
