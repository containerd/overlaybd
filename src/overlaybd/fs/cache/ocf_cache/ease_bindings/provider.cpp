...
int ease_ocf_provider::start(bool reload_media) {
    ...
    m_cfg.device.volume_type = EASE_OCF_VOLUME_TYPE;
    m_cfg.core.volume_type = EASE_OCF_VOLUME_TYPE;
    ...
    LOG_INFO("OCF: OCF cache is ready, blk_size `, prefetch_unit `", m_volume_params->blk_size, m_prefetch_unit);
    return 0;
}
...
int ease_ocf_provider::stop() {
    ...
    if (m_cache != nullptr) {
        ...
        simple_ctx.sem.wait(1);
        if (simple_ctx.error != 0) {
            LOG_ERROR_RETURN(0, -1, "OCF: failed to stop management cache, error = `", simple_ctx.error);
        } else {
...
void ease_ocf_provider::prepare_aligned_iov(size_t count, off_t offset, alignment& a, IOVector& iov,
    ...
    LOG_DEBUG("bound `-`, padding `-`, mid `, iov cnt `, size `",
              a.lower_bound, a.upper_bound, a.lower_padding, a.upper_padding, a.mid_count, iov.iovcnt(), iov.sum());
}
...
ssize_t ease_ocf_provider::ocf_pread(void* buf, size_t count, off_t offset, size_t blk_addr, OcfSrcFileCtx* ctx,
                                     bool prefetch) {
    ...
    DEFER({ if (padding_buf != nullptr) g_io_alloc->dealloc(padding_buf); });
