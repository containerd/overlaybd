...
static int prefetch_read(OcfSrcFileCtx* src_file_ctx, size_t count, off_t offset, off_t blk_addr) {
    ...
    if (new_count == 0 || (size_t) new_offset >= src_file_ctx->ns_info.file_size) {
        ...
        new_count = ROUND_UP(src_file_ctx->ns_info.file_size, ease_ocf_provider::SectorSize) - new_offset;
    }
    ...
    auto ret = src_file_ctx->provider->ocf_pread(buf, new_count, new_offset, blk_addr, src_file_ctx);
    if (ret <= 0) {
...
static int volume_open(ocf_volume_t volume, void* volume_params) {
    ...
    if (strncmp((const char*) vol->uuid.data, "core", vol->uuid.size) == 0) {
        // For now the volume_open of core will always receive a nullptr volume_params, so blk_size can only
        // be known from io_data. Maybe OCF will change it in the future.
    ...
    if (strncmp((const char*) vol->uuid.data, "cache", vol->uuid.size) == 0) {
        if (vol->cache_params->enable_logging) {
            LOG_DEBUG("OCF cache `, buf `, count: `, offset: `",
                      io->dir == OCF_READ ? "read" : "write", local_iov.iovec()[0].iov_base, local_iov.sum(), io->addr);
        }
        ...
        LOG_DEBUG("OCF core `, buf `, count: `, offset: `",
                  io->dir == OCF_READ ? "read" : "write", local_iov.iovec()[0].iov_base, local_iov.sum(), offset);
        if (io->dir == OCF_WRITE) {
            ret = -1;
            errno = ENOSYS;
        } else {
...
int volume_init(ocf_ctx_t ocf_ctx) {
    return ocf_ctx_register_volume_type(ocf_ctx, EASE_OCF_VOLUME_TYPE, &volume_properties);
}

void volume_cleanup(ocf_ctx_t ocf_ctx) {
    ocf_ctx_unregister_volume_type(ocf_ctx, EASE_OCF_VOLUME_TYPE);
}
