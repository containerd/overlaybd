...
static ctx_data_t* ctx_data_alloc(uint32_t pages) {
    ...
    if (buf == nullptr) {
        LOG_ERRNO_RETURN(ENOMEM, nullptr, "OCF: failed to allocate memory, size `", PAGE_SIZE * pages);
    }
...
inline static uint64_t ctx_data_copy(ctx_data_t* dst, ctx_data_t* src, uint64_t to, uint64_t from, uint64_t bytes) {
    ...
    LOG_DEBUG("OCF: ctx data copy: src buf `, src seek_offset `, dst buf `, dst seek_offset `, copy ` bytes",
              data_src->iovs[0].iov_base, data_src->seek, data_dst->iovs[0].iov_base, data_dst->seek, bytes);
