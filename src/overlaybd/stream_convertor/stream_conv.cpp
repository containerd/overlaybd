...
public:
    ...
    int read_buffer(http::Request &req, http::Response &resp, std::string_view){
        Task t;
        auto body_size = req.headers.content_length();
        LOG_DEBUG("body size: `", body_size);
        resp.set_result(200);

        std::string str = "{\"code\": 0, \"message\": \"\"}\n";
        if (body_size == 0) {
            str = "{\"code\": -1, \"message\": \"invalid body size(0)\"}\n";
    ...
    int start() {
        ...
        m_http_serv->add_handler({this, &StreamConvertor::read_buffer});
