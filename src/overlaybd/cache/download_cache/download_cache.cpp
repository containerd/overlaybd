...
class DownloadCacheFs;

class DownloadCacheStore : public ForwardFile_Ownership {
public:
    DownloadCacheStore(IFile *file, DownloadCacheFs *fs)
...
class DownloadCacheFile : public VirtualFile {
public:
    DownloadCacheFile(IFile *file, const char *pathname, DownloadCacheFs *fs)
...
private:
    ...
    DownloadCacheStore *m_local_file = nullptr;
    size_t m_size = 0;
    bool ready = false;
    std::string m_name;
    DownloadCacheFs *m_fs;
...
public:
    ...
    ~DownloadCacheFs() {
        LOG_INFO("delete DownloadCacheFs");
    }
    virtual IFile *open(const char *pathname, int flags) override {
        ...
        return new DownloadCacheFile(src_file, pathname, this);
    }
    ...
    size_t refill_size;
    IOAlloc *io_alloc;
    ObjectCache<std::string, DownloadCacheStore *> m_file_pool;
...
ssize_t DownloadCacheFile::preadv(const struct iovec *iov, int iovcnt, off_t offset) {
    ssize_t ret = 0;
    if (!ready) {
...
int DownloadCacheFile::fstat(struct stat *buf) {
    return m_file->fstat(buf);
}

int DownloadCacheFile::vioctl(int request, va_list args) {
    if (request == SET_LOCAL_DIR) {
        ...
        m_local_file = m_fs->m_file_pool.acquire(m_local_path, [&]() -> DownloadCacheStore * {
            ...
            return new DownloadCacheStore(f, m_fs);
        });
...
int DownloadCacheFile::fallocate(int mode, off_t offset, off_t len) {
    if (m_local_file == nullptr) {
...
std::pair<off_t, size_t> DownloadCacheStore::query_refill_range(off_t offset, size_t size) {
    off_t align_left = align_down(offset, m_fs->block_size);
...
IFileSystem *new_download_cached_fs(photon::fs::IFileSystem *src_fs, size_t blk_size,
                                    size_t refill_size, IOAlloc *io_alloc) {
    if (io_alloc == nullptr) {
        io_alloc = new IOAlloc;
    }
    return new ::Cache::DownloadCacheFs(src_fs, blk_size, refill_size, io_alloc);
}
} // namespace FileSystem
