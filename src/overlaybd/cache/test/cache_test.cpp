...
void* worker(void* arg) {
  ...
  for (int i=0;i<4;i++) {
    ...
    for (const auto &x : offset) {
      ::pread(fd, buffersrc, 1024*1024, x);
...
TEST(CachedFS, write_while_full) {
  std::string srcRoot("/tmp/ease/cache/src_test/");
  SetupTestDir(srcRoot);
  system("dd if=/dev/urandom of=/tmp/ease/cache/src_test/huge bs=1M count=2048");
...
TEST(CachedFS, fn_trans_func) {
  std::string srcRoot("/tmp/ease/cache/src_test/");
  SetupTestDir(srcRoot);
  system("mkdir /tmp/ease/cache/src_test/path_aaa/");
  system("mkdir /tmp/ease/cache/src_test/path_bbb/");
  system("dd if=/dev/urandom of=/tmp/ease/cache/src_test/path_aaa/sha256:test bs=1K count=1");
  system("cp /tmp/ease/cache/src_test/path_aaa/sha256:test /tmp/ease/cache/src_test/path_bbb/sha256:test");
...
int main(int argc, char** argv) {
  ...
  int ret = photon::fd_events_init(photon::INIT_EVENT_EPOLL);
  if (ret < 0) {
    LOG_ERROR_RETURN(0, -1, "failed to init epoll subsystem");
  }

  ret = photon::libaio_wrapper_init();
  if (ret < 0)
