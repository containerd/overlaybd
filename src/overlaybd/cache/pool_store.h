...
public:
    ICachePool(uint32_t pool_size = 128, uint32_t max_refilling = 128,
               uint32_t refilling_threshold = -1U);
    ~ICachePool();
    ...
    virtual ssize_t list(const char *dirname, ListType type, const struct iovec *iov, int iovcnt,
                         const char *marker, uint32_t count) {
        errno = ENOSYS;
        return -1;
    }
...
protected:
    ...
    void *m_vcpu = nullptr; // vcpu where m_therad_pool is created
...
public:
    ...
    virtual int set_crc(uint32_t crc) {
        errno = ENOSYS;
        return -1;
    }
    virtual int get_crc(uint32_t *crc) {
        errno = ENOSYS;
        return -1;
    }
    virtual uint64_t get_handle() {
        return -1UL;
    }
    ...
    void release() {
        ...
        if (ref == 1 && pool_) {
            ...
            delete this; // call do_open directly
    }
    ...
    std::string_view get_src_name() {
        return src_name_;
    }
    void set_src_name(std::string_view pathname) {
        src_name_ = pathname.data();
    }
    std::string_view get_store_key() {
        return store_key_;
    }
    void set_store_key(std::string_view pathname) {
        store_key_ = pathname;
    }
    void set_pool(ICachePool *pool) {
        pool_ = pool;
    }
    void set_cached_size(off_t cached_size);
    off_t get_actual_size() {
        return actual_size_;
    }
    void set_actual_size(off_t actual_size) {
        actual_size_ = actual_size;
    }
    void set_open_flags(int open_flags) {
        open_flags_ = open_flags;
    }
    int open_src_file(photon::fs::IFile **src_file = nullptr);
    void set_src_file(photon::fs::IFile *src_file) {
        src_file_ = src_file;
    }
    photon::fs::IFileSystem *get_src_fs() {
        return src_fs_;
    }
    void set_src_fs(photon::fs::IFileSystem *src_fs) {
        src_fs_ = src_fs;
    }
    ...
    void set_allocator(IOAlloc *allocator) {
    ...
    virtual try_preadv_result try_preadv2(const struct iovec *iov, int iovcnt, off_t offset,
                                          int flags);
...
private:
    ...
    ssize_t do_refill_range(uint64_t refill_off, uint64_t refill_size, size_t count,
...
public:
    virtual ssize_t pin_buffer(off_t offset, size_t count, int flags, /*OUT*/ iovector *iov,
                               void **pin_result) = 0;
...
public:
    ...
    virtual ssize_t pin_buffer(uint64_t handle, off_t offset, size_t count, int flags,
                               /*OUT*/ iovector *iov, void **pin_result) = 0;
