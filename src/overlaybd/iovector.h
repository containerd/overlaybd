...
struct iovector_view {
    ...
    explicit constexpr iovector_view(iovec *iov, int iovcnt) : iov(iov), iovcnt(iovcnt) {
    }

    template <int N>
    explicit constexpr iovector_view(iovec (&iov)[N]) : iov(iov), iovcnt(N) {
    }
    ...
    const iovec *begin() const {
        return iov;
    }
    const iovec &front() const {
        return *iov;
    }
    const iovec *end() const {
        return iov + iovcnt;
    }
    const iovec &back() const {
        return iov[iovcnt - 1];
    }
...
protected:
    ...
    inline __attribute__((always_inline)) struct iovec *iovs_ptr() const {
    ...
    IOVAllocation_ *
    allocator() const { // this makes an assumption of the memory layout as `IOVectorEntity`
    ...
    struct iovec new_iovec(size_t size_) {
        do_assert();
        assert(nbases < capacity);
        int size = (size_ <= INT_MAX) ? (int)size_ : INT_MAX;
...
public:
    ...
    explicit IOVectorEntity(uint16_t preserve = DEF_PRESERVE) : iovector(CAPACITY, preserve) {
    }

    explicit IOVectorEntity(const struct iovec *iov, int iovcnt, uint16_t preserve = DEF_PRESERVE)
    ...
    explicit IOVectorEntity(IOAlloc allocator, uint16_t preserve = DEF_PRESERVE)
    ...
    IOVectorEntity(IOVectorEntity &&rhs) : iovector(CAPACITY, DEF_PRESERVE) {
        *(iovector *)this = std::move(rhs);
    }
    IOVectorEntity(iovector &&rhs) : iovector(CAPACITY, DEF_PRESERVE) {
        *(iovector *)this = std::move(rhs);
    }
...
inline void do_static_assert() {
    ...
    static_assert(offsetof(IOV, allocator) + offsetof(IOAlloc, bases) == offsetof(IOV, bases),
                  "offset assumption");
#endif
}
...
inline iovector *new_iovector(uint16_t capacity, uint16_t preserve) {
    auto size =
        sizeof(iovector) + sizeof(iovec) * capacity + sizeof(IOAlloc) + sizeof(void *) * capacity;
...
public:
    ...
    SmartCloneIOV(const iovec *iov, int iovcnt) {
        ptr = (iovcnt <= (int)CAPACITY) ? this->iov : new iovec[iovcnt];
