...
static int figure_loop_size(struct loop_device *lo, loff_t offset,
			    loff_t sizelimit)
{
...
static int figure_loop_size_mfile(struct loop_device *lo, loff_t offset,
				  loff_t sizelimit)
{
	//loff_t size = get_size(offset, sizelimit, lo->lo_backing_file);
...
size_t lsmt_iter_pread(struct lsmt_ro_file *file, struct iov_iter *iter,
				    ...
				    loff_t pos)
{
	...
	rq_for_each_segment (bvec, rq, iter) {
		...
		pr_info("Before lsmt_iter_pread, pos is %lx, bvec.bv_len is %lx ",
			pos, bvec.bv_len);
...
static int lo_discard_mfile(struct loop_device *lo, struct request *rq,
			    loff_t pos)
{
	BUG_ON(1);
	/*
...
static void lo_complete_rq(struct request *rq)
{
	...
	pr_info("cmd->ret is %d\n", cmd->ret);
	pr_info("blk_rq_bytes is %lu\n", blk_rq_bytes(rq));
	pr_info("cmd->use_aio is %d\n ", cmd->use_aio);
...
{
	pr_info("in loop_validate_file before\n");
	struct inode *inode = file->f_mapping->host;
	pr_info("in loop_validate_file after\n");
	struct file *f = file;
...
/* 
* get fd from userspace and init lsmt_ro_file 
* note that we will fput rather than filp_close when we call loop_clr_fd_mfile to release resources
* hence there is no need to call loop_destroy_lsmtfile
*/

static struct lsmt_ro_file *
loop_init_lsmtfile(struct loop_device *lo,
		   const struct loop_mfile_fds __user *arg)
{
	...
	if (!access_ok(VERIFY_READ, arg, sizeof(struct loop_mfile_fds)))
		return NULL;
	if (copy_from_user(&kfds, arg, sizeof(struct loop_mfile_fds)))
		return NULL;
	IMAGE_RO_LAYERS = kfds.mfcnt;
	pr_info("Now we have %d files in lsmt_ro_file \n", IMAGE_RO_LAYERS);

	mfds = kvmalloc(sizeof(struct loop_mfile_fds) +
				sizeof(int) * IMAGE_RO_LAYERS,
			GFP_KERNEL);
	if (!access_ok(VERIFY_READ, arg->fds, sizeof(int) * IMAGE_RO_LAYERS))
		return NULL;
	//now copy all the mem from userspace
	if (copy_from_user(mfds, arg,
			   sizeof(struct loop_mfile_fds) +
				   sizeof(int) * IMAGE_RO_LAYERS))
		return NULL;
	...
	for (i = 0; i < IMAGE_RO_LAYERS; i++) {
		...
		if (!files[i]) {
			pr_info("Fail to convert from fd to struct file *\n");
			return NULL;
		}
	}
	ro = open_files((void **)files, IMAGE_RO_LAYERS, true);
	pr_info("Created lsmt_ro_file object. addr: 0x%lx",
	...
	pr_info("file->MAX_IO_SIZE: %lu", ro->MAX_IO_SIZE);
	kvfree(files);
	return ro;
}
...
static void loop_alloc_mfile(struct loop_mfile *mf, size_t fcnt)
{
	...
	mf->mfcnt = fcnt;
	for (i = 0; i < fcnt; i++) {
		mf->filenames[i] = (char *)kvzalloc(LO_NAME_SIZE * sizeof(char),
						    GFP_KERNEL);
	}
}
...
static void loop_copy_mfile(const struct loop_mfile *src,
			    struct loop_mfile *dst)
{
	size_t n = src->mfcnt;
	size_t i;
	...
	for (i = 0; i < n; i++) {
		pr_info("src->filenames[%lu] is %s, LO_NAME_SIZE is %d\n", i,
			src->filenames[i], LO_NAME_SIZE);
		//memcpy((char *)src->filenames[i], (char *)dst->filenames[i], LO_NAME_SIZE);
		memcpy((char *)dst->filenames[i], (char *)src->filenames[i],
		       LO_NAME_SIZE);
			     ...
			     const struct loop_mfile_fds __user *arg)
{
	...
	mapping = ((struct file *)lsmtfile->m_files[0])->f_mapping;
	...
	lo->lo_backing_files =
		(struct file **)
			lsmtfile->m_files; //for operating in loop_clr_fd_mfile
	lo->transfer = NULL;
	lo->ioctl = NULL;
	lo->lo_sizelimit = 0;
	lo->old_gfp_mask = mapping_gfp_mask(mapping);
	...
	if (!(lo_flags & LO_FLAGS_READ_ONLY) &&
	    ((struct file *)lsmtfile->m_files[0])->f_op->fsync)
		blk_queue_write_cache(lo->lo_queue, true, false);
...
out_putf:
	...
	for (i = 0; i < n; i++) {
		fput(lsmtfile->m_files[i]);
	}
	module_put(THIS_MODULE);
	return error;
out:
	pr_info("lsmt file initialization failed\n");
	module_put(THIS_MODULE);
	return error;
}
...
static int loop_set_status_mfile(struct loop_device *lo,
				 const struct loop_info64 *info)
{
	int err;
	struct loop_func_table *xfer;
	...
	if (lo->lo_offset != info->lo_offset ||
	    lo->lo_sizelimit != info->lo_sizelimit) {
		if (figure_loop_size_mfile(lo, info->lo_offset,
					   info->lo_sizelimit)) {
			err = -EFBIG;
			goto exit;
		}
...
static int loop_get_status_mfile(struct loop_device *lo,
...
static int loop_set_status64_mfile(struct loop_device *lo,
				   const struct loop_info64 __user *arg)
{
	struct loop_info64 info64;
	size_t i = 0;
	...
	for (i = 0; i < n; i++) {
		...
		if (!access_ok(VERIFY_READ, (char *)info64.mfile.filenames[i],
			       LO_NAME_SIZE))
			return NULL;
		//if(copy_from_user((char *)fns[i], arg->mfile.filenames[i], LO_NAME_SIZE)) return NULL;
		if (copy_from_user((char *)fns[i],
				   (char *)info64.mfile.filenames[i],
				   LO_NAME_SIZE))
			return NULL;
	}
...
static int loop_get_status64_mfile(struct loop_device *lo,
				   struct loop_info64 __user *arg)
{
	struct loop_info64 info64;
	int err = 0;
	...
	n = info64.mfile.mfcnt;
	for (i = 0; i < n; i++) {
		if (copy_to_user(arg->mfile.filenames[i],
				 info64.mfile.filenames[i], LO_NAME_SIZE))
			return -EFAULT;
	}
		    ...
		    unsigned long arg)
{
	...
	case LOOP_SET_FD_MFILE:
		pr_info("here am i \n");
		err = loop_set_fd_mfile(lo, mode, bdev,
					(struct loop_mfile_fds __user *)arg);
		break;
	case LOOP_CHANGE_FD:
		pr_info("calling LOOP_CHANGE_FD\n");
	...
	case LOOP_SET_STATUS:
		...
		if ((mode & FMODE_WRITE) || capable(CAP_SYS_ADMIN))
			err = loop_set_status_old(
				lo, (struct loop_info __user *)arg);
		break;
	...
	case LOOP_SET_STATUS64:
		err = -EPERM;
		if ((mode & FMODE_WRITE) || capable(CAP_SYS_ADMIN))
			err = loop_set_status64(
				lo, (struct loop_info64 __user *)arg);
		break;
	case LOOP_SET_STATUS64_MFILE:
		err = -EPERM;
		if ((mode & FMODE_WRITE) || capable(CAP_SYS_ADMIN))
			err = loop_set_status64_mfile(
				lo, (struct loop_info64 __user *)arg);
		break;
	...
	case LOOP_GET_STATUS64_MFILE:
		err = loop_get_status64_mfile(lo,
					      (struct loop_info64 __user *)arg);
		/* loop_get_status() unlocks lo_ctl_mutex */
		goto out_unlocked;
